...
module Feature
  ...
  module PlainStepToEol0
    def line_to_eol
      elements[1]
    end
  end

  def _nt_plain_step_to_eol
    start_index = index
    if node_cache[:plain_step_to_eol].has_key?(index)
      cached = node_cache[:plain_step_to_eol][index]
      @index = cached.interval.end if cached
      return cached
    end
    ...
    if s0.last
      r0 = (SyntaxNode).new(input, i0...index, s0)
      r0.extend(PlainStepToEol0)
    else
      self.index = i0
      r0 = nil
    end

    node_cache[:plain_step_to_eol][start_index] = r0
