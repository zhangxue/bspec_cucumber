...
module Cucumber
  class CLI
    ...
    attr_reader :options
    FORMATS = %w{pretty profile progress html autotest report fw_html}
  ...
  * #{cucumber_yml.keys.join("\n  * ")}
    ...
    def build_formatter_broadcaster(step_mother)
      ...
      @options[:formats].each do |format, output_list|
        ...
        when 'report'
          formatter_broadcaster.register(Formatters::ReportFormatter.new(output_broadcaster, step_mother))
        when 'fw_html'
          formatter_broadcaster.register(Formatters::FWHtmlFormatter.new(output_broadcaster, step_mother))
        else
          raise "Unknown formatter: #{format}"
        end
      end
